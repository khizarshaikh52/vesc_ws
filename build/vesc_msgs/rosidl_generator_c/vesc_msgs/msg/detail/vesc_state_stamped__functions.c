...
vesc_msgs__msg__VescStateStamped__create(void)
{
  ...
  vesc_msgs__msg__VescStateStamped__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    vesc_msgs__msg__VescStateStamped * data =
      (vesc_msgs__msg__VescStateStamped *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!vesc_msgs__msg__VescStateStamped__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          vesc_msgs__msg__VescStateStamped__fini(&output->data[i]);
        }
        return false;
      }
    }

...
vesc_msgs__msg__VescImuStamped__create(void)
{
  ...
  vesc_msgs__msg__VescImuStamped__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    vesc_msgs__msg__VescImuStamped * data =
      (vesc_msgs__msg__VescImuStamped *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!vesc_msgs__msg__VescImuStamped__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          vesc_msgs__msg__VescImuStamped__fini(&output->data[i]);
        }
        return false;
      }
    }
